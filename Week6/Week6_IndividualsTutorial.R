...
# The authors used ANOVAs to test the significance of different potential 'predators' in generating a flight response among squirrels
  ...
  # They did not, however consider individual variation in the models - i.e. some squirrels are scaredy cats.
...
# Now that we have a good model, let's check the residuals for homogeniety:
plot(glmm.mod2)
...
# gam() also allows you to model how two predictor variables effect the y variable simultaneously.
  # This is one of the rare times when I will approve of a 3D plot:
